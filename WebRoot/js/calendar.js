...
Calendar.prototype.draw = function() {
	...
	mvAry[mvAry.length] = '      <tr>';
	for ( var i = 0; i < 7; i++) {
		mvAry[mvAry.length] = '      <th style="font-weight:normal;font-size:10px;background-color:'
	...
	mvAry[mvAry.length] = '        <th colspan="2"><input name="calendarClear" type="button" id="calendarClear" value="'
			...
			+ ';background-color:'
			+ calendar.colors["input_bg"]
			+ ';width:100%;height:20px;font-size:10px;"/></th>';
	...
	str += "    <input radix=\"24\" name=\"c_hours\"  id= \"c_hours\" value=\"\""
			+ sMinute_Common + ">:"
	str += "    <input radix=\"60\" name=\"c_minutes\" id= \"c_minutes\" value=\"\""
			+ sMinute_Common + ">:"
	str += "    <input radix=\"60\" name=\"c_seconds\" id= \"c_seconds\" value=\"\""
			+ sMinute_Common + ">"
	str += "   </div>"
	str += "  </td>"
	str += "  <td>"
	str += "  <table border=\"0\" cellspacing=\"2\" cellpadding=\"0\">"
	str += "   <tr><td><button id=\"c_up\" " + sButton_Common
			+ ">5</button></td></tr>"
	str += "   <tr><td><button id=\"c_down\" " + sButton_Common
			+ ">6</button></td></tr>"
	str += "  </table>"
	str += "  </td>"
	str += " </tr>"
...
Calendar.prototype.goNextMonth = function(e) {
	...
	this.date = new Date(this.year, this.month, 1, this.hours, this.minutes,
			this.seconds);
	this.changeSelect();
	this.bindData();
}
		...
		if (dateArray[i] != "&nbsp;") {
			tds[i].onclick = function() {
				if (calendar.dateControl != null) {
					...
					this.date = new Date(calendar.date.getFullYear(),
							calendar.date.getMonth(), this.innerHTML,
							this.hours, this.minutes, this.seconds);
					calendar.dateControl.value = this.date
			...
			if (calendar.dateControl != null
					&& calendar.dateControl.value == new Date(calendar.date
							...
							dateArray[i], c_hours.value, c_minutes.value,
							c_seconds.value).format(calendar.dateFormatStyle)) {
...
document
		.write('background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:10px;"></div>');
if (document.all) {
	document
...
Calendar.prototype.playback = function() {
	...
	var arrDate = [ objDate.getHours(), objDate.getMinutes(),
			objDate.getSeconds() ];
