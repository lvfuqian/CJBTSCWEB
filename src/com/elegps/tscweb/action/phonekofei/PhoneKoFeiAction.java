package com.elegps.tscweb.action.phonekofei;
...
public class PhoneKoFeiAction extends BaseAction{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (user != null) {
			...
			if (cmd.equals("pkf_search")) {// 增加群组信息
				actionforward = phonekfList(mapping, form, request, response);
			}
		} else {
	...
	private ActionForward phonekfList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
...
//		HttpSession session = request.getSession(true);
		...
		int pkfCount = 0;
		pkfCount = pkManager.findPKFCountByPage(pagentid, childagentid, ep, msid, Integer.valueOf(type), startTime, endTime,imsi);
		// 获取每页的条数
		...
		// 获取总页数
		int pageCount = (pkfCount + pageSize - 1) / pageSize;
		// 从页面取得当前页
		int pageNo;
		...
		// 获取总条数
		request.setAttribute("kfCount", pkfCount);
		// 返回每页面显示条数
...
//                .getChildAgentByParamentid(pagentid));
		...
		List<TbPhonekoufeiLog> kfList = null;
		if(pkfCount>0){
			kfList = pkManager.findPKFListInfo_sertchByPage(pageNo, pageSize, pagentid, childagentid, ep,
