...
import emdx.cn.b2m.eucp.sdkhttp.Mo;
import emdx.cn.b2m.eucp.sdkhttp.StatusReport;

public class TestSDKClient {
	public static String softwareSerialNo = "8SDK-EMY-6699-RFYUP";// 软件序列号,请通过亿美销售人员获取
	public static String key = "DHXX";// 序列号首次激活时自己设定
	public static String password = "917080";// 密码,请通过亿美销售人员获取
...
//		testSendSMS();
	...
	/**
	 * 请用正规内容来测试，短信内容里面不能出现：       测试，test等词语
	 * 
	...
	public static void testSendSMS() {
		...
		try {
			int i = SingletonClient.getClient().sendSMS(new String[] { "13802922524" }, "【德亨信息】尊敬的用户欢迎使用app", "",5);// 带扩展码
	...
	public static void testSendSMSAddMessageId() {
		try {
			int i = SingletonClient.getClient().sendSMSEx(new String[] { "13802922524" }, "带有信息ID的短信", "", "GBK", 5, 123456789);
			System.out.println("testsSendScheduledSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
		}
			...
			if (list != null) {
				...
				while (it.hasNext()) {
					...
					System.out.println("手机号码:" + report.getMobile() + "\t状态:"
							+ report.getReportStatus() + "\t信息ID：" + report.getSeqID() + "\t错误信息：" + report.getErrorCode()+report.getMemo()+report.getServiceCodeAdd());
				}
			} else {
